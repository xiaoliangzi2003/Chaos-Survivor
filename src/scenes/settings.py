...
class SettingsScene(Scene):
    def on_enter(self, **kwargs) -> None:
        self._title_font = get_font(54, bold=True)
        self._row_font = get_font(24, bold=True)
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        surface.blit(subtitle, subtitle.get_rect(centerx=SCREEN_WIDTH // 2, y=94))

        selected = SETTING_DEFINITIONS[self._selected]
        detail = f"{selected.description} 默认值 {selected.default:.2f}x"
        for line_idx, line in enumerate(wrap_text(self._small_font, detail, 1080, max_lines=2)):
            text = self._small_font.render(line, True, (175, 192, 225))
            surface.blit(text, text.get_rect(centerx=SCREEN_WIDTH // 2, y=132 + line_idx * 22))
        ...
        for idx, (meta, rect) in enumerate(zip(SETTING_DEFINITIONS, self._row_rects())):
            ...
            surface.blit(value_text, value_text.get_rect(center=(rect.centerx + 170, rect.centery)))

            minus_rect, plus_rect = self._adjust_buttons(rect)
            self._draw_adjust_button(surface, minus_rect, "-", selected_row)
            self._draw_adjust_button(surface, plus_rect, "+", selected_row)

        reset_rect = self._reset_button()
        back_rect = self._back_button()
        self._draw_action_button(surface, reset_rect, "恢复默认 (R)", (80, 185, 255))
        self._draw_action_button(surface, back_rect, "返回菜单", (255, 200, 100))

        footer = self._small_font.render("上下选择，左右调整，鼠标也可点击加减按钮。", True, (170, 180, 210))
        surface.blit(footer, footer.get_rect(centerx=SCREEN_WIDTH // 2, y=SCREEN_HEIGHT - 74))

        message = self._small_font.render(self._message, True, (220, 230, 255))
        surface.blit(message, message.get_rect(centerx=SCREEN_WIDTH // 2, y=SCREEN_HEIGHT - 44))
    ...
    def _row_rects(self) -> list[pygame.Rect]:
        ...
        row_h = 40
        gap = 8
        return [pygame.Rect(120, start_y + idx * (row_h + gap), 1040, row_h) for idx in range(len(SETTING_DEFINITIONS))]

    def _adjust_buttons(self, rect: pygame.Rect) -> tuple[pygame.Rect, pygame.Rect]:
        minus_rect = pygame.Rect(rect.right - 132, rect.y + 4, 34, 32)
        plus_rect = pygame.Rect(rect.right - 44, rect.y + 4, 34, 32)
        return minus_rect, plus_rect

    def _reset_button(self) -> pygame.Rect:
        return pygame.Rect(SCREEN_WIDTH // 2 - 220, SCREEN_HEIGHT - 118, 170, 42)

    def _back_button(self) -> pygame.Rect:
        return pygame.Rect(SCREEN_WIDTH // 2 + 50, SCREEN_HEIGHT - 118, 170, 42)
